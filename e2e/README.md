...
2.  **Safety Check (`TestMain`)**: Before running any tests, the suite verifies that the target cluster's control-plane IP matches the `controlPlaneIP` specified in the test manifest (e.g., `internal/controllers/tpunodegroup/testdata/test_nodegroup.yaml`). This prevents accidentally running tests against the wrong cluster.
...
```bash
go test -v -tags=e2e ./e2e/... -run TestTPUNodeGroup
```
